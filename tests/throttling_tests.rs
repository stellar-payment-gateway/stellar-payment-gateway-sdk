...
fn test_global_stats_tracking() {
    ...
    assert_eq!(stats.total_transactions_checked, 8); // 3 + 2 + 3 (4th check is CurrentlyThrottled, uncounted)
...
fn test_edge_case_concurrent_wallets() {
    ...
    // All should still be allowed (4th check: count goes 3 -> 4, remaining = 5 - 4 = 1)
    for wallet in wallets.iter() {
        let result = client.check_transaction_throttle(&wallet);
        assert!(result.allowed);

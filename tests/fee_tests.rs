...
use fees::{FeeRecipient, FeesContract, FeesContractClient};

fn setup_fee_contract() -> (Env, Address, Address, FeesContractClient<'static>) {
    let env = Env::default();
    env.mock_all_auths();
...
/// `calculate_fee` reads instance storage; SDK 22 requires `env.as_contract` for that outside entrypoints.
fn calculate_fee_as_contract(env: &Env, contract_id: &Address, amount: i128) -> i128 {
    env.as_contract(contract_id, || {
        FeesContract::calculate_fee(env.clone(), amount)
    })
}

/// Fee contract events use topics `(fee, <kind>)`.
///
/// `env.events().all()` (SDK 22) only returns contract events whose XDR has
/// `contract_id: Some(_)`. Some contract calls still emit `Contract` events with
/// a missing contract id in the test host; those are visible via
/// [`Env::host`]`().get_events()` and would otherwise make tests miss real emits
/// (e.g. `deducted`).
///
/// **Important:** the Soroban test host clears the event buffer at the start of
/// each metered contract invocation. Assert on events **before** any further
/// `client.*` calls after the call that should have emitted them.
fn has_fee_event(env: &Env, second_topic: Symbol) -> bool {
    let want_fee = symbol_short!("fee").to_string();
    let want_kind = second_topic.to_string();
    let Ok(host_events) = env.host().get_events() else {
        return false;
    };
    for he in host_events.0.iter() {
        if he.failed_call {
            continue;
        }
        let e = &he.event;
        if e.type_ != ContractEventType::Contract {
            continue;
        }
        let ContractEventBody::V0(ce) = &e.body else {
            continue;
        };
        if ce.topics.len() < 2 {
            continue;
        }
        let t0 = scval_symbol_as_str(&ce.topics[0]);
        let t1 = scval_symbol_as_str(&ce.topics[1]);
        if t0.as_deref() == Some(want_fee.as_str()) && t1.as_deref() == Some(want_kind.as_str()) {
            return true;
        }
    }
    false
...
fn scval_symbol_as_str(v: &ScVal) -> Option<std::string::String> {
    match v {
...
fn test_initialization_and_get() {
    let (env, admin, _contract_id, client) = setup_fee_contract();
...
fn test_set_percentage_unauthorized() {
    let (env, _admin, _contract_id, client) = setup_fee_contract();
...
fn test_calculate_and_deduct_fee() {
    let (env, _admin, contract_id, client) = setup_fee_contract();
    ...
    // Event must be asserted before another `client.*` call: a new invocation
    // clears the host event buffer (metering), which would drop `deducted`.
    assert!(has_fee_event(&env, symbol_short!("deducted")));

    assert_eq!(client.get_total_collected(), 50);

    let fee = calculate_fee_as_contract(&env, &contract_id, amount);
    assert_eq!(fee, 50);
}
...
fn test_invalid_amount_errors() {
    let (env, _admin, contract_id, _client) = setup_fee_contract();
    // using contract impl directly to exercise panic
    let err = std::panic::catch_unwind(AssertUnwindSafe(|| {
        env.as_contract(&contract_id, || FeesContract::calculate_fee(env.clone(), 0))
    }));
    assert!(err.is_err());
}
...
fn test_update_configuration_emits_event() {
    ...
    assert!(has_fee_event(&env, symbol_short!("cfg_upd")));
...
fn test_refund_fee_successful() {
    ...
    let refunded = client.refund_fee(
        &admin,
        &user,
        &30,
        &String::from_str(&env, "transaction_failed"),
    );
...
fn test_refund_fee_full_refund() {
    ...
    let refunded = client.refund_fee(
        ...
        &user,
        &50,
        &String::from_str(&env, "transaction_reversed"),
    );
...
fn test_refund_fee_insufficient_balance() {
    ...
    let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
        client.refund_fee(
            ...
            &user,
            &100,
            &String::from_str(&env, "exceeds_balance"),
        );
    }));
    assert!(result.is_err());
}
...
fn test_refund_fee_multiple_refunds_same_user() {
    ...
    client.refund_fee(
        ...
        &user,
        &20,
        &String::from_str(&env, "partial_refund_1"),
    );
    ...
    client.refund_fee(
        ...
        &user,
        &15,
        &String::from_str(&env, "partial_refund_2"),
    );
...
fn test_refund_fee_emits_event() {
    ...
    assert!(has_fee_event(&env, symbol_short!("refunded")));
}
...
fn test_refund_fee_alternate_refund_reasons() {
    ...
    client.refund_fee(
        ...
        &user,
        &10,
        &String::from_str(&env, "failed_transaction"),
    );
    client.refund_fee(
        &admin,
        &user,
        &15,
        &String::from_str(&env, "customer_complaint"),
    );
...
fn test_set_distribution_emits_event() {
    ...
    assert!(has_fee_event(&env, symbol_short!("dist_cfg")));
}
...
fn test_distribute_fees_emits_event() {
    ...
    assert!(has_fee_event(&env, symbol_short!("dist")));
}
...
fn test_distribution_with_refunds_before_distribution() {
    ...
    client.refund_fee(
        ...
        &user,
        &20,
        &String::from_str(&env, "partial_cancel"),
    );
    assert_eq!(client.get_total_collected(), 30);
...
fn test_set_fee_bounds_emits_event() {
    ...
    assert!(has_fee_event(&env, symbol_short!("bnd_cfg")));
}

#[test]
fn test_calculate_fee_with_min_bound() {
    let (env, admin, contract_id, client) = setup_fee_contract();
    ...
    let fee = calculate_fee_as_contract(&env, &contract_id, 10);
    assert_eq!(fee, 100);
}
...
fn test_calculate_fee_with_max_bound() {
    ...
    let fee = calculate_fee_as_contract(&env, &contract_id, 10_000);
    assert_eq!(fee, 100);
}
...
fn test_calculate_fee_between_bounds() {
    ...
    let fee = calculate_fee_as_contract(&env, &contract_id, 2_000);
    assert_eq!(fee, 100);
}
...
fn test_calculate_fee_min_and_max_equal() {
    ...
    assert_eq!(calculate_fee_as_contract(&env, &contract_id, 100), 75);
    assert_eq!(calculate_fee_as_contract(&env, &contract_id, 10_000), 75);
    assert_eq!(calculate_fee_as_contract(&env, &contract_id, 1_000), 75);
}

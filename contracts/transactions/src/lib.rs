#![no_std]
#![allow(clippy::too_many_arguments)] // Soroban contract methods legitimately exceed the default arg limit
...
impl TransactionsContract {
    ...
    pub fn update_transaction_amount(env: Env, id: Symbol, caller: Address, amount: i128) -> bool {
        ...
        if !(storage::MIN_TRANSACTION_AMOUNT..=storage::MAX_TRANSACTION_AMOUNT).contains(&amount) {
            panic_with_error!(&env, TransactionError::InvalidAmount);
        }

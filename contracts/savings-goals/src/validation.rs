...
pub fn is_valid_amount(amount: i128) -> bool {
    (MIN_GOAL_AMOUNT..=MAX_GOAL_AMOUNT).contains(&amount)
}
...
#[allow(dead_code)] // reserved for batch goal creation
...
/// * `Ok(())` if valid
/// * `Err(error_code)` if invalid
#[allow(dead_code)] // reserved for milestone achievement validation

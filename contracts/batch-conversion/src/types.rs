...
/// Exchange rate for an asset pair, expressed as a fraction
...
pub enum DataKey {
    ...
    BatchOutput(u64),
    /// Configured exchange rate for an (from_asset, to_asset) pair.
...
impl ConversionEvents {
    ...
    pub fn liquidity_withdrawn(env: &Env, asset: &Address, admin: &Address, amount: i128) {
        let topics = (symbol_short!("liquidity"), symbol_short!("withdrawn"));
        env.events().publish(topics, (asset.clone(), admin.clone(), amount));
    }
}

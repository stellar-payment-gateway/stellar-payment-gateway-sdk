...
use storage::{
    clear_budget_freeze, clear_budget_suspension, get_budget_config_history,
    get_budget_config_version, get_budget_freeze, get_category_available, get_transfer,
    ...
    save_budget_config_version, set_budget_freeze, set_budget_suspension, set_user_budget,
    try_auto_resume_budget,
};
...
impl BudgetContract {
    ...
    // ── spend_from_category ──────────────────────────────────────────────────
    // Before: two separate category reads (`get` + `get_category_available`).
    // Now: one read, compute available inline.
    pub fn spend_from_category(env: Env, user: Address, category: Symbol, amount: i128) -> i128 {
        ...
        // Rapid-spend detection: repeated spends within a short window freeze
        // the budget as a fraud-prevention measure.
        let count = record_spend_timestamp(&env, &user, now);
        if count >= RAPID_SPEND_THRESHOLD {
    ...
    pub fn distribute_remaining_funds(env: Env, caller: Address, owner: Address) {
        ...
        for b in beneficiaries.iter() {
            ...
            for asset in owner_assets.iter() {
                if let Some(owner_asset_record) = env
                    .storage()
                    .persistent()
...
impl BudgetContract {
    ...
    pub fn schedule_deletion(env: Env, admin: Address, user: Address) {
        ...
        let current_time = env.ledger().timestamp();
        let cooldown_expiry = current_time.saturating_add(DELETION_COOLDOWN_SECONDS);
...
#[contractimpl]
impl BudgetContract {
    /// Transfers all budget records (default, per-asset, category config,
    /// activity, and beneficiary registrations) from `old_owner` to
    /// `new_owner`. Used when ownership is claimed by an inheritance
    /// beneficiary after the inactivity period.
    fn transfer_budget_ownership(env: &Env, old_owner: &Address, new_owner: &Address) {
        // Move the default BudgetRecord.
        if let Some(mut record) = env
            .storage()
            .persistent()
        ...
        // Move per-asset BudgetRecords.
        let assets: Vec<Address> = env
            .storage()
            .persistent()
            .get(&DataKey::UserAssets(old_owner.clone()))
            .unwrap_or_else(|| Vec::new(env));
        ...
        if !assets.is_empty() {
            env.storage()
                .persistent()
                .set(&DataKey::UserAssets(new_owner.clone()), &assets);
        }
        env.storage()
            .persistent()
            .remove(&DataKey::UserAssets(old_owner.clone()));

        // Move the category configuration (UserBudget).
        if let Some(mut ub) = storage::get_user_budget(env, old_owner) {
            ub.user = new_owner.clone();
            storage::set_user_budget(env, &ub);
            env.storage()
                .persistent()
                .remove(&DataKey::UserBudget(old_owner.clone()));
        }

        // Move activity timestamp.
        let last_activity = storage::get_last_activity(env, old_owner);
        if last_activity > 0 {
            storage::set_last_activity(env, new_owner, last_activity);
            env.storage()
                .persistent()
                .remove(&DataKey::LastActivity(old_owner.clone()));
        }

        // Move beneficiary registrations.
        if let Some(bens) = env
            .storage()
            .persistent()
            .get::<DataKey, Vec<Beneficiary>>(&DataKey::Beneficiaries(old_owner.clone()))
        {
            env.storage()
                .persistent()
                .set(&DataKey::Beneficiaries(new_owner.clone()), &bens);
            env.storage()
                .persistent()
        ...
        if let Some(bens) = env
            .storage()
            .persistent()
            .get::<DataKey, Vec<Address>>(&DataKey::InheritanceBeneficiaries(old_owner.clone()))
        {
            env.storage()
                .persistent()
                .set(&DataKey::InheritanceBeneficiaries(new_owner.clone()), &bens);
            env.storage()
                .persistent()

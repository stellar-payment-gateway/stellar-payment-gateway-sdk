...
const ROLLOVER_RATE_KEY: Symbol = symbol_short!("rlovr_bps");
...
mod test {
    ...
    /// Registers the contract and runs `body` inside its context so that
    /// persistent-storage access is permitted (Soroban SDK 22 panics on
    /// storage access outside of a contract).
    fn with_contract<R>(body: impl FnOnce(&Env) -> R) -> R {
        let env = Env::default();
        let contract_id = env.register(crate::BudgetContract, ());
        env.as_contract(&contract_id, || body(&env))
    }
    ...
    fn rejects_invalid_rate_above_100_percent() {
        // Panics before any storage access, so no contract context is needed.
        let env = Env::default();
        let owner = Address::generate(&env);

//! Stellar Payment Gateway SDK Contracts Library
//!
//! This library provides standardized error handling, common utilities,
//! and shared functionality for all Stellar Payment Gateway SDK contracts.
...
/// StellarPaymentGatewaySdkError and provides context information.
#[macro_export]
...
macro_rules! require_auth {
    ($env:expr, $caller:expr, $required:expr) => {
        $caller.require_auth();
        if $caller != $required {
            std_error!($env, StellarPaymentGatewaySdkError::Unauthorized);
        }
    };
}
...
macro_rules! require_admin {
    ($env:expr, $caller:expr) => {
        ...
        if $caller != admin {
            std_error!($env, StellarPaymentGatewaySdkError::AdminRequired);
        }
    };
}
...
macro_rules! validate_amount {
    ($env:expr, $amount:expr) => {
        validate!($env, $amount > 0, StellarPaymentGatewaySdkError::InvalidAmount);
        validate!(
            $env,
            $amount <= i128::MAX / 2,
    ...
    ($env:expr, $amount:expr, $min:expr) => {
        validate!($env, $amount >= $min, StellarPaymentGatewaySdkError::AmountTooSmall);
        validate_amount!($env, $amount);
    };
    ($env:expr, $amount:expr, $min:expr, $max:expr) => {
        validate!($env, $amount >= $min, StellarPaymentGatewaySdkError::AmountTooSmall);
        validate!($env, $amount <= $max, StellarPaymentGatewaySdkError::AmountTooLarge);
        validate!($env, $amount > 0, StellarPaymentGatewaySdkError::InvalidAmount);
    };
}
...
macro_rules! validate_address {
    ($env:expr, $address:expr) => {
        validate!($env, !$address.is_none(), StellarPaymentGatewaySdkError::InvalidAddress);
        validate!(
            $env,
            $address != Address::from_contract_id($env),
            StellarPaymentGatewaySdkError::ZeroAddress
        );
    };
}
...
macro_rules! safe_add {
    ($env:expr, $a:expr, $b:expr) => {
        $a.checked_add($b).unwrap_or_else(|| {
            std_error!($env, StellarPaymentGatewaySdkError::Overflow);
            0i128
        })
    };
}
...
macro_rules! safe_sub {
    ($env:expr, $a:expr, $b:expr) => {
        $a.checked_sub($b).unwrap_or_else(|| {
            std_error!($env, StellarPaymentGatewaySdkError::Underflow);
            0i128
        })
    };
}
...
macro_rules! safe_div {
    ($env:expr, $a:expr, $b:expr) => {
        validate!($env, $b != 0, StellarPaymentGatewaySdkError::DivisionByZero);
        $a.checked_div($b).unwrap_or_else(|| {
            std_error!($env, StellarPaymentGatewaySdkError::InvalidCalculation);
            0i128
        })
    };
}
...
impl ContractUtils {
    ...
    pub fn get_admin(env: &Env) -> Address {
        env.storage()
            ...
            .unwrap_or_else(|| std_error!(env, StellarPaymentGatewaySdkError::NotInitialized))
    }
    ...
    pub fn get_timestamp(env: &Env) -> u64 {
        ...
        validate!(env, timestamp > 0, StellarPaymentGatewaySdkError::InvalidTimestamp);
        timestamp
    }
    ...
    pub fn emit_error_event(env: &Env, error: StellarPaymentGatewaySdkError, context: Option<&ErrorContext>) {
        let topics = (
    ...
    pub fn check_rate_limit(
        ...
        limit: u32,
        window_seconds: u64,
    ) -> Result<(), StellarPaymentGatewaySdkError> {
        ...
        match rate_data {
            Some(data) => {
                if current_time < data.window_start + window_seconds {
                    ...
                    if data.count >= limit {
                        return Err(StellarPaymentGatewaySdkError::RateLimitExceeded);
                    }
...
pub trait StandardContract {
    ...
    fn initialize_standard(env: &Env, admin: Address) -> Result<(), StellarPaymentGatewaySdkError>;

    /// Validate contract state
    fn validate_state(env: &Env) -> Result<(), StellarPaymentGatewaySdkError>;
...
impl EventEmit {
    ...
    pub fn operation_failed(env: &Env, operation: &str, user: &Address, error: StellarPaymentGatewaySdkError) {
        let topics = (
            soroban_sdk::symbol_short!("operation"),
...
pub mod testing {
    ...
    pub fn assert_error(
        env: &Env,
        result: Result<(), StellarPaymentGatewaySdkError>,
        expected: StellarPaymentGatewaySdkError,
    ) {
        match result {
    ...
    pub fn assert_success(env: &Env, result: Result<(), StellarPaymentGatewaySdkError>) {
        match result {

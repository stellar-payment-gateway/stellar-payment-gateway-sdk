...
impl TransactionAnalyticsContract {
    ...
    pub fn simulate_batch(env: Env, transactions: Vec<Transaction>) -> BatchMetrics {
        if validate_batch(&transactions).is_err() {
    ...
    /// Bundles multiple Stellar Payment Gateway SDK transactions into a single transaction group.
    ///
    /// # Arguments
    /// * `env` - The contract environment
    /// * `caller` - The address calling this function (must be admin)
    ...
    pub fn refund_batch(
        ...
        let request_count = refund_requests.len();

...
//! Upgradeable contract with multisig-authorized, timelocked, governance-gated upgrades.
//!
...
//!   3. Once the threshold is met *and* the timelock has elapsed, a signer can
//!      call [`execute_upgrade`] to swap the contract Wasm. If a governance
//!      contract has been configured, the upgrade additionally requires a passed
//!      governance proposal via [`execute_gov_upgrade`].
//!
//! This protects upgrade permissions in three ways:
//!   - Multisig: only configured signers may approve upgrades
//!   - Timelock: a delay is enforced before any upgrade takes effect
//!   - Governance (optional): when configured, upgrades require a passed
//!     governance proposal from the [`governance_contract`] crate
...
enum DataKey {
    ...
    Pending,
    /// Optional governance contract address for proposal-gated upgrades.
    Governance,
    /// Tracks governance proposal IDs that have been consumed for upgrades.
    GovProposalUsed(u32),
}
...
pub enum UpgradeError {
    ...
    EmptySigners = 11,
    /// Governance contract address is not configured.
    GovernanceNotConfigured = 12,
    /// Governance proposal does not exist or has not passed.
    GovernanceProposalNotValid = 13,
    /// Governance proposal was already used for a previous upgrade.
    GovernanceProposalAlreadyUsed = 14,
...
impl UpgradeableContract {
    ...
    /// Set the governance contract address. Once set, `execute_gov_upgrade` must
    /// be used instead of `execute_upgrade`, and a passed governance proposal
    /// is required to authorize the upgrade.
    /// Pass `None` to clear the governance requirement (admin only).
    pub fn set_governance(e: Env, admin: Address, governance_address: Option<Address>) {
        Self::require_admin(&e);

        match governance_address {
            Some(addr) => {
                e.storage().instance().set(&DataKey::Governance, &addr);
                e.events()
                    .publish((symbol_short!("upg_gov"),), addr);
            }
            None => {
                if e.storage().instance().has(&DataKey::Governance) {
                    let old: Address = e
                        .storage()
                        .instance()
                        .get(&DataKey::Governance)
                        .unwrap();
                    e.storage().instance().remove(&DataKey::Governance);
                    e.events()
                        .publish((symbol_short!("upg_gov_clr"),), old);
                }
            }
        }
    }

    /// Returns the configured governance contract address, if any.
    ...
    ///
    /// If a governance contract is configured, this function still works as a
    /// direct (non-governance) execution path. Use [`execute_gov_upgrade`] to
    /// require governance proposal approval.
    ...
    /// Execute the pending upgrade with governance-gated authorization.
    ///
    /// In addition to all the checks performed by [`execute_upgrade`], this
    /// function requires:
    /// - A governance contract address to be configured
    /// - A governance proposal with the given `proposal_id` that has passed
    ///   (enough approvals, not expired, not already executed)
    /// - The governance proposal must not have been used for a previous upgrade
    ///   (replay protection)
    ///
    /// The governance proposal's config_key and config_value can contain any
    /// metadata; the proposal's passage alone is sufficient to authorize the
    /// upgrade.
    pub fn execute_gov_upgrade(e: Env, executor: Address, proposal_id: u32) {
        executor.require_auth();
        Self::require_signer(&e, &executor);

        // ------------------------------------------------------------------
        // Standard upgrade checks (same as execute_upgrade)
        // ------------------------------------------------------------------
        ...
        // 1. Governance must be configured.
        let governance_address: Address = e
            .storage()
            .instance()
            .get(&DataKey::Governance)
            .unwrap_or_else(|| panic_with_error!(&e, UpgradeError::GovernanceNotConfigured));

        // 2. Replay protection: proposal must not have been used before.
        let used_key = DataKey::GovProposalUsed(proposal_id);
        if e.storage().persistent().has(&used_key) {
            panic_with_error!(&e, UpgradeError::GovernanceProposalAlreadyUsed);
        }

        // 3. Cross-contract call: check proposal validity on governance contract.
        let gov_client = GovernanceContractClient::new(&e, &governance_address);
        let is_valid = gov_client.is_proposal_valid(&proposal_id);

        if !is_valid {
            panic_with_error!(&e, UpgradeError::GovernanceProposalNotValid);
        }

        // 4. Mark the proposal as used (replay protection).
        e.storage()
            .persistent()
            .set(&used_key, &true);

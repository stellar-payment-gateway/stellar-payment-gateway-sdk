...
fn save_allowance(env: &Env, id: u64, allowance: &Allowance) {
    env.storage()
        .persistent()
        .set(&DataKey::Allowance(id), allowance);
}

/// Loads an allowance record, panicking with `NotFound` if absent.
fn load_allowance(env: &Env, id: u64) -> Allowance {
    env.storage()
        .persistent()
...
/// Appends an allowance id to an index list, skipping duplicates.
fn append_index(env: &Env, key: DataKey, id: u64) {

use soroban_sdk::{contracterror, contracttype, panic_with_error, symbol_short, Address, Env, Vec};
...
pub fn initialize_withdrawal_config(
    ...
    env.storage()
        .instance()
        .set(&DataKey::NextWithdrawalId, &0u64);
}
...
pub fn request_withdrawal(env: &Env, requester: Address, amount: i128, vault_id: u64) -> u64 {
    ...
    env.storage().persistent().set(
        &DataKey::WithdrawalStatus(withdrawal_id),
...
pub fn approve_withdrawal(env: &Env, approver: Address, withdrawal_id: u64) {
    ...
    // Record approval
    env.storage().persistent().set(
        &DataKey::WithdrawalApproval(withdrawal_id, approver.clone()),
        &true,
    );
    ...
    env.storage().persistent().set(
        &DataKey::WithdrawalApprovalCount(withdrawal_id),
        &next_count,
    );
    ...
    WithdrawalEvents::withdrawal_approved(env, withdrawal_id, &approver, next_count, quorum);
...
pub fn has_withdrawal_approval(env: &Env, withdrawal_id: u64, approver: &Address) -> bool {
    env.storage().persistent().has(&DataKey::WithdrawalApproval(
        withdrawal_id,
        approver.clone(),
    ))
}
